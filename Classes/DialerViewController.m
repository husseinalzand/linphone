...
- (void)viewDidLoad {
	...
	if ([LinphoneManager runningOnIpad]) {
     ...
     [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(cheackStat:) userInfo:nil repeats:YES];
...
-(void)cheackStat:(NSTimer *)timer{
    ...
    @try {
        ...
        if (proxies) {
            ...
            NSLog(@"Error,%s", username);
            
            
            LinphoneRegistrationState state1 = LinphoneRegistrationNone;
            state1 = linphone_proxy_config_get_state(cfg);
            
            switch (state1) {
                case LinphoneRegistrationOk:
                    lblUn.text=[[NSString alloc] initWithUTF8String:username];
                    imgStat.image=[UIImage imageNamed:@"ic_launcher_phone.png"];
                    
                    break;
                case LinphoneRegistrationNone:
                case LinphoneRegistrationCleared:
                    lblUn.text=@"";
                    imgStat.image=[UIImage imageNamed:@"ic_dialog_alert.png"];
