...
const LinphoneCallParams *params = linphone_call_get_remote_params(call);
    const char *a=linphone_call_params_get_custom_header(params, "X-DoorUrl");

    
    NSLog(@"%s is header",a);
    [avatarImage setImage:[[UIImage alloc] initWithData:nil]];
    input=nil;
    
    DataClass *obj=[DataClass getInstance];
    obj.str=NULL;
    obj.allowOpenDoor=NO;
    if (a != NULL) {
               
        //DataClass *obj=[DataClass getInstance];
        obj.str=[[NSMutableString alloc] initWithUTF8String:a];
        obj.allowOpenDoor=YES;
        
        
        
        NSMutableString *input1 = [[NSMutableString alloc] initWithUTF8String:a];
        [input1 insertString:@"/jpeg" atIndex:[input1 length]];
    
    input=[NSString stringWithString:input1];
        
        
        /*NSURL *url = [NSURL URLWithString:input1];
        ...
        [NSURLConnection sendAsynchronousRequest:urlRequest queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
        {
            ...
            else
            {
                ...
                avatarImage.image = [[UIImage alloc] initWithData:data];
                [avatarImage setImage:[[UIImage alloc] initWithData:data]];
            }
        }];*/
    ...
    [avatarImage setImage:[[UIImage alloc] initWithData:data]];
       ...
       /* NSData * imageData = [[NSData alloc] initWithContentsOfURL: [NSURL URLWithString: @"http://192.168.1.207:8080/jpeg"]];
        while (imageData==NULL) {
            NSLog(@"%s is header","boş");
        }
...
-(void)tapDetected{
    ...
    NSData * imageData = [[NSData alloc] initWithContentsOfURL: [NSURL URLWithString:input]];
    [avatarImage setImage:[UIImage imageWithData: imageData]];
