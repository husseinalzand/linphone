...
- (void)viewWillAppear:(BOOL)animated {
    ...
    avatar.image=NULL;
    NSMutableString *openDoor;
    DataClass *obj=[DataClass getInstance];
   
    
    if (obj.allowOpenDoor) {
        NSString *openDoor1=obj.str;
        
        openDoor = [NSMutableString stringWithString:openDoor1];
        [openDoor insertString:@"/jpeg" atIndex:[openDoor length]];
        
        
        NSURL *url = [NSURL URLWithString:openDoor];
        NSData *dataa = [NSData dataWithContentsOfURL:url];
        [avatar setImage:[[UIImage alloc] initWithData:dataa]];
        opebDoorOutlet.hidden=NO;
    }else{
        opebDoorOutlet.hidden=YES;
    }
...
- (IBAction)openDoor:(id)sender {
    ...
    NSString *openDoor1=obj.str;
     ...
     openDoor = [NSMutableString stringWithString:openDoor1];
    [openDoor insertString:@"/opendoor" atIndex:[openDoor length]];
    ...
    /*
    linphone_core_send_dtmf([LinphoneManager getLc], '1');
    linphone_core_play_dtmf([LinphoneManager getLc], '1', 100);
    sleep(1);
    linphone_core_send_dtmf([LinphoneManager getLc], '3');
    linphone_core_play_dtmf([LinphoneManager getLc], '3', 100);
    sleep(1);
    linphone_core_send_dtmf([LinphoneManager getLc], '*');
