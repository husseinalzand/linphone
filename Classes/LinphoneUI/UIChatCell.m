...
- (void)update {
	...
	if (displayName == nil) {
		...
		ms_free(address);
        int count = linphone_chat_room_get_history_size(chatRoom);
        displayName=[NSString stringWithFormat:@"%@ (%d)",displayName,count];
	...
	// Avatar
	if (image == nil) {
		image = [UIImage imageNamed:@"ic_contact_picture_180_holo_dark.png"];
	}
    ...
    [lblDate setText:[dateFormatter stringFromDate:message_date]];
